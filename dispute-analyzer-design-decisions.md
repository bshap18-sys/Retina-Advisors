...
Stripe category (top-level reason field, one of the seven categories), granular network reason code (Visa / Mastercard / Amex code on payment_method_details.card.network_reason_code, e.g. Visa 10.4, MC 4837), amount, currency, evidence due date, dispute status, dispute case type (chargeback / inquiry / block), Visa Compelling Evidence 3.0 eligibility status with required-actions detail, is_charge_refundable flag, balance transactions array (the actual debit and any reversal with timestamps - load-bearing for refund-in-flight detection per principle 9), payment_intent linkage, dispute filing timestamp (created). Note: customer name, customer email, billing address, and purchase IP do NOT live as ground-truth fields on the dispute object - they are pulled from the charge object in Layer 2. The fields that look similar on the dispute object (under evidence.*) are merchant-supplied evidence slots, null by default. See dispute-analyzer-stripe-field-map.md for exact field paths.

Layer 2 - Stripe charge object expansion (automatic pull via Stripe MCP - second call):
Customer name (billing_details.name), customer email (billing_details.email and receipt_email - both worth capturing, mismatch is itself a signal), billing address (billing_details.address - six subfields), purchase IP (best-effort, may be null - lives on underlying PaymentIntent or Radar session, not directly on the charge), ECI indicator, wallet type (apple_pay / google_pay / link / samsung_pay / visa_checkout / etc., null = standard card), card fingerprint, AVS result (Stripe exposes this as two separate checks - address_line1_check and address_postal_code_check, each pass / fail / unavailable / unchecked - the analyzer combines them into a verdict), CVC result, 3DS authentication outcome (result plus authentication_flow of challenge / frictionless and version), 3DS exemption indicator (affects liability shift assessment), card brand, card funding type (credit / debit / prepaid), card country (issuing country - cross-border purchase signal for friendly fraud), card last4, network transaction ID, network token used flag (CE3-relevant signal), Radar risk score (0-100), Radar risk level (normal / elevated / highest), Radar rule that fired if any, Radar session ID (radar_options.session - browser metadata snapshot, supports identity coherence per principle 3), Stripe outcome seller_message (plain English verdict like "Payment complete" or "Stripe blocked this payment as fraudulent"), Stripe fraud_details (stripe_report = Stripe's authorization-time fraud assessment, user_report = any prior merchant report of safe / fraudulent), merchant metadata on the charge (order ID, product info), refund history on the charge including timestamps. See dispute-analyzer-stripe-field-map.md for exact field paths.
